  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        {activeUser && (
          <button
            ...
            className="p-1 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-rose-500 transition-all flex-shrink-0"
          >
      ...
      <div className="flex items-start gap-2.5 text-xs text-slate-500 mb-4 leading-relaxed">
        <MapPin className="w-4 h-4 text-mint-600 flex-shrink-0 mt-0.5" />
        <div>
          <p className="text-slate-800 font-semibold">{farmacia.local_direccion}</p>
          <p className="text-slate-400 font-mono text-[10px]">
            {farmacia.comuna_nombre} {farmacia.localidad_nombre ? `(${farmacia.localidad_nombre})` : ''}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-3 border-t border-slate-100 text-xs mb-3">
        <div className="flex items-center gap-1.5 text-slate-500 font-mono">
          ...
          <span className="font-semibold">{farmacia.funcionamiento_hora_apertura.slice(0, 5)} - {farmacia.funcionamiento_hora_cierre.slice(0, 5)}</span>
        </div>
